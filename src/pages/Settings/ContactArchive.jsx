...
const ContactArchive = () => {
  ...
  const fetchData = () => {
    axios
      .get(`${BASE_URL}/archive-listing?search=${search}&type=contact&sort_direction=${sortDirection}&sort_by=${sortBy}`, config)
      .then((res) => {
        const value = res?.data?.contactlist;
                ...
                <tr className="uppercase body-N dark-M">
                  ...
                  <th>
                    <div className="table-header">
                      ...
                      <img
                        role="button"
                        src={sortBy !== "last_name" ? swap : sortDirection === "desc" ? sortDesc : sortAsce}
                        onClick={() => {
                          setSortBy("last_name");
                          if (sortDirection === "desc") {
                            setSortDirection("asc");
              ...
              <tbody>
                {listData?.map((el, idx) => (
                  <tr key={idx} className="dark-H body-N">
                    ...
                    <td className="capitalize">{el?.first_name} </td>
                    <td className="capitalize">{el?.last_name}</td>
