...
const EditPropertyTypeModal = ({ showModal, onClose, onCallApi, id }) => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${BASE_URL}/property-type-getbyid/${id}`, config)
        .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    setDisable(true);
    axios
      .post(`${BASE_URL}/edit-property-type/${id}`, { type: type }, config)
      .then((res) => {
        onCallApi();
        handleClose();
        if (res?.data?.message) {
  ...
  return (
    <Modal title={"Edit  Property Type"} desc={"This property type will be accessible for item owned by Tri-Oak Consulting Group"} show={showModal} onClose={handleClose}>
