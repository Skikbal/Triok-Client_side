...
const AddPropertyTagModal = ({ showModal, onClose, onCallApi }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setDisable(true);
    axios
      .post(`${BASE_URL}/create-propertytag`, { tag_name: tagName }, config)
      .then((res) => {
        onCallApi();
        handleClose();
        if (res?.data?.message) {
  ...
  return (
    <Modal title={"Add Property Tag"} desc={"This tag will be accessible for item owned by Tri-Oak Consulting Group"} show={showModal} onClose={handleClose}>
...
export default AddPropertyTagModal;
