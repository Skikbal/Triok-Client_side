...
import AddPropertyTagModal from "./modals/PropertyTagModals/AddPropertyTagModal";
import EditPropertyTagModal from "./modals/PropertyTagModals/EditPropertyTagModal";
...
const PropertyTags = () => {
  ...
  const fetchContacts = () => {
    axios
      .get(`${BASE_URL}/propertytag-listing?page=${currentPage}&per_page=${itemPerPage}&search=${search}&sort_direction=${sortDirection}&sort_by=${sortBy}`, config)
      .then((res) => {
        console.log("res", res);
        const value = res?.data?.propertytag;
  ...
  const handleDelete = () => {
    axios
      .delete(`${BASE_URL}/delete-propertytag/${selectedId}`, config)
      .then((res) => {
        setSelectedId("");
        onSuccess();
  ...
  return (
    <BaseLayout>
      ...
      <div className="light-bg-L pb-3 table-container">
                ...
                <tr className="uppercase body-N dark-M">
                  ...
                  <th className="green-H">
                    <div className="table-header">
                      ...
                      <img
                        role="button"
                        src={sortBy !== "tag_name" ? swap : sortDirection === "desc" ? sortDesc : sortAsce}
                        onClick={() => {
                          setSortBy("tag_name");
                ...
                {listData?.map((el, idx) => (
                  <tr key={idx} className="dark-H body-N">
                    ...
                    <td className="flex items-center gap-2">
                      <p className="tags green-H body-S">{el?.tag_name}</p>
                    </td>
        ...
        <AddPropertyTagModal showModal={showModal} onClose={() => setShowModal(false)} onCallApi={onSuccess} />

        <EditPropertyTagModal
