...
const BuyerArchive = () => {
  ...
  const fetchData = () => {
    axios
      .get(`${BASE_URL}/archive-listing?search=${search}&type=buyer&sort_direction=${sortDirection}&sort_by=${sortBy}`, config)
      .then((res) => {
        const value = res?.data?.buyerlist;
              ...
              <tbody>
                {listData?.map((el, idx) => (
                  <tr key={idx} className="dark-H body-N">
                    ...
                    <td className="capitalize"> {el.contact?.name}</td>
                    <td className="capitalize">{el?.property_type?.[0]?.type}</td>
