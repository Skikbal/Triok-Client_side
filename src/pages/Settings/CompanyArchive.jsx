...
const CompanyArchive = () => {
  ...
  const fetchData = () => {
    axios
      .get(`${BASE_URL}/archive-listing?search=${search}&type=company&sort_direction=${sortDirection}&sort_by=${sortBy}`, config)
      .then((res) => {
        const value = res?.data?.companylist;
              ...
              <tbody>
                {listData?.map((el, idx) => (
                  <tr key={idx} className="dark-H body-N">
                    ...
                    <td className="capitalize">{el?.company_name} </td>
