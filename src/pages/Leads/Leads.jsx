...
const Leads = () => {
  ...
  const [filterData, setFilterData] = useState(initialLeadFilterData);
  ...
  const buildQueryParams = (filters) => {
    ...
    const { bds, broker, lead_source, created_at, created_end_date, created_start_date, contact, lead_type, link_id, property, buyer_id } = filters;
  ...
  return (
    <BaseLayout>
      ...
      <div className="light-bg-L pb-3 table-container">
              ...
              <tbody>
                {leadsData?.map((el, idx) => (
                  <tr key={idx} className="dark-H body-N">
                    ...
                    <td className="dark-M">{el?.lead_source?.name ?? "N/A"}</td>
                    <td className="dark-M">{moment(el?.created_at).format("MM/DD/YY")}</td>
                    <td
                    ...
                    >
                      <img src={contact} alt="contact icon" className="icon-class w-4 h-4" /> {el?.contact?.first_name} {el?.contact?.last_name}
                    </td>
                    ...
                    <td className="flex gap-3 items-center w-full justify-center">
                      <ActionsMenu
                        ...
                        otherOptionTitle={"Convert To Proposal"}
                        handleOtherOption={() => {
        ...
        {leadsData?.length > 0 && <ContactPagination paginationData={paginationData} handleNext={handleNext} handlePrev={handlePrev} onSuccess={onSuccess} />}
      </div>
